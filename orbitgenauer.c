/**
 * orbit_processor.c (V5.10 - Aggregator-Version)
 *
 * V5.10 FIX: Behebt zwei fatale C-Compiler-Fehler:
 * 1. 'a.size' in free_orbit_array wurde zu 'a->size' korrigiert (Pointer-Zugriff)
 * 2. 'add_orbit(&final_orbits)' wurde zu 'add_orbit(final_orbits)' korrigiert
 * (Entfernt doppelte Adressnahme, 'final_orbits' war bereits ein Pointer)
 *
...
typedef struct {
    ...
    double elev;
    time_t timestamp;
    Vector3 world_pos; // Berechnete 3D-Position
} DataPoint;
...
Vector3 v_apply_y_rotation(Vector3 v, double angle) {
    double cos_a = cos(angle);
    double sin_a = sin(angle);
    Vector3 v_out;
    v_out.x = v.x * cos_a + v.z * sin_a;
    v_out.y = v.y;
    v_out.z = v.z * cos_a - v.x * sin_a;
    return v_out;
}

double getEarthRotationFromTimestamp(time_t timestamp) {
    struct tm *tm = gmtime(&timestamp); 
    double totalSecondsInDay = (tm->tm_hour * 3600) + (tm->tm_min * 60) + tm->tm_sec;
    double fractionOfDay = totalSecondsInDay / 86400.0; 
    return (fractionOfDay * 2.0 * PI);
}

Vector3 calculateWorldPos(
    Vector3 static_observer_pos, 
    time_t timestamp,            
    double azim, 
    double elev, 
    double orbit_radius) 
{
    double earth_rotation = getEarthRotationFromTimestamp(timestamp);
    Vector3 observer_world_pos = v_apply_y_rotation(static_observer_pos, earth_rotation);
    Vector3 userUpVector = v_normalize(observer_world_pos);
    ...
    double a = 1.0;
    double b = 2.0 * v_dot(observer_world_pos, sightLineDirection);
    double c = v_mag_sq(observer_world_pos) - (orbit_radius * orbit_radius);
    double discriminant = (b * b) - (4.0 * a * c);

    if (discriminant >= 0) {
        double t = (-b + sqrt(discriminant)) / (2.0 * a);
        return v_add(observer_world_pos, v_scale(sightLineDirection, t));
    } else {
...
void free_orbit_array(OrbitArray *a) {
    free(a->orbits);
    a->orbits = NULL;
    // V5.10 FIX: 'a' ist ein Zeiger, benutze '->' statt '.'
    a->size = a->capacity = 0; 
}
...
// Verarbeitet die Pässe (unverändert von V5.9)
void process_satellite_passes(OrbitArray *final_orbits, PointArray *points, const char* sat_id) {
    ...
    // V5.10 FIX: 'final_orbits' ist bereits ein Zeiger, das '&' war falsch.
...
int main() {
    ...
    Vector3 static_observer_pos = getPositionFromLatLon(OBSERVER_LAT, OBSERVER_LON, VISUAL_EARTH_RADIUS);
    ...
    while (fgets(line, sizeof(line), f_csv)) {
        ...
        p.world_pos = calculateWorldPos(static_observer_pos, p.timestamp, p.azim, p.elev, r);
